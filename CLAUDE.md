...
**증상 (Fit Result 탭)**:
- R² = -0.01 (음수 — 평균보다 나쁜 피팅)
- R1 = R2 = 0.001 mΩ (하한값에 붙음)
- C1 = 500 F (상한), C2 ≈ 1995 F (상한 근처)
- 피팅 곡선(빨간 점선)이 Vp2 근처에 수평선으로 그려짐
- 측정 전압(파란 점)이 40초 동안 상승 후 하강 (CC 전압 + 이완 전압 혼합)

**근본 원인** (`preprocessor.py` — `prepare_fit_data`):
```python
# 수정 전
n_samples = max(2, int(round(window_s / dt)))   # dt = 글로벌 median
```
BioLogic 파일이 DCIM 버스트(dt ≈ 1ms)와 CC 데이터(dt ≈ 0.1s)가 섞여 있을 때
글로벌 median dt는 버스트 구간의 빠른 간격(≈1ms)을 채택.
`5s / 0.001s = 5000 샘플`을 추출하지만 실제로 이 5000개는 약 **40초** 분량을 차지.
→ 피팅 윈도우가 CC 구간을 넘어 이완 구간(전류 차단 후 전압 하강)까지 캡처.
→ 2RC 모델은 단조 증가 전압을 가정하므로 상승→하강 패턴을 맞출 수 없음.
→ 옵티마이저가 R1, R2 → 0 (하한), C1, C2 → 상한으로 도망 → 피팅 완전 실패.

**수정 내용** (`preprocessor.py`):
- `prepare_fit_data`, `prepare_joint_fit_data`, `prepare_relaxation_data` 세 함수 모두:
- 샘플 수 기반 → **실제 시간 기반** 윈도우 추출로 교체
- `np.searchsorted(t - t_p2, window_s)` 로 `window_s` 초까지의 샘플만 포함
- 시간 리셋(BioLogic 시퀀스 경계)은 `np.diff < 0` 감지로 첫 역방향 점프 직전에서 중단 → searchsorted 안전성 보장
- 혼합 샘플링 레이트 데이터에서도 정확히 `window_s` 초만 캡처

**수정 내용** (`app.py` — 사이드바 CSS):
- `[data-testid="stSidebar"] * { color: #E0EAF5 !important; }` 블랭킷 규칙 제거
- 원인: 모든 요소에 흰 글씨 강제 → selectbox 안 선택값이 흰 배경 위 흰 글씨가 되어 안 보임
- 수정: `p`, `span`, `li`, `label` 만 밝은 색, `[data-baseweb="select"]` 등 위젯 입력 영역은 `#1A1A1A` (어두운 글씨) 명시 적용
- 라디오/체크박스 라벨도 명시 셀렉터로 분리하여 선택 상태 대비 확보

**수정 내용** (`app.py` — 분석 파이프라인 게이지):
- `st.spinner()` 6개 → `st.progress()` 1개로 통합
- 단계별 퍼센트: 로딩 5% → EIS 15% → p탐지 25% → Rs 40% → 데이터 준비 52% → 피팅 63% → 나이퀴스트 88% → 완료 100%
- 각 단계 텍스트에 ①~⑦ 번호 + 설명 표시 → 사용자가 어느 단계에서 멈춰 있는지 시각적으로 파악 가능
- 피팅 단계("가장 오래 걸립니다") 명시로 긴 대기 시 혼동 방지

**수정 후 기대 효과**:
- 피팅 윈도우가 의도한 `window_s` 초만 정확히 캡처 → CC 구간만으로 RC 파라미터 수렴
- R² > 0.99 복구, R1/R2가 물리적 범위 내로 수렴
- 사이드바 selectbox 선택값이 모든 위젯에서 명확히 보임
- 분석 실행 시 진행률이 한눈에 보여 응답성 체감 개선
