...
def export_results_excel(
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        ...
        param_rows = [
            ...
            ("R3 [Ω]",  result.R3,       result.sigma_R3),
            ("C3 [F]",  result.C3,       result.sigma_C3),
            ("sigma_W [Ω s^-1/2]", result.sigma_W, result.sigma_sigma_W),
            ("τ1 [s]",  result.tau1,     "—"),
            ("τ2 [s]",  result.tau2,     "—"),
            ("τ3 [s]",  result.tau3,     "—"),
            ("f1 [Hz]", result.f1,       "—"),
...
def export_report_text(result: FitResult) -> str:
    ...
    lines = [
        ...
        f"  R3  = {result.R3 * 1000:.4f} mΩ  ±  {result.sigma_R3 * 1000:.4f} mΩ",
        f"  C3  = {result.C3:.4f} F      ±  {result.sigma_C3:.4f} F",
        f"  σW  = {result.sigma_W:.6g} Ω·s^(-1/2)",
        "",
        "── Fit Quality ──",
        f"  R²   = {result.r2:.6f}",
